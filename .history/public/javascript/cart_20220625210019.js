...
$(document).ready(function () {
  ...
  carts.forEach((cart, index) => {
    ...
    let cart_html = `
    <div class="product-cart__list ${cart.id}">
      <div class="row">
        <div class="col l-4">
...
module.increase = function () {
  $(document).on("click", ".cart_item_increase", function (event) {
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res.code == 200) {
          module.settings.list_carts = res.data.cart_items;
          module.settings.container.list_cart.html("");

          let list_cart = Handlebars.compile(
            module.settings.template.list_cart.html()
          );

          module.settings.container.list_cart.append(
            list_cart({ cart_items: module.settings.list_carts })
          );
        } else {
