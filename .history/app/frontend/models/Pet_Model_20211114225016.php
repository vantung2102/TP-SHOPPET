<?php
    ...
    class Pet_Model extends Base_Model{
        ...
        function processSortDesc($offset,  $per_page){
        ...
        function processSortAsc($offset,  $per_page){
            $query = "select * from {$this->table_pet} ORDER BY new_price ASC limit $offset, $per_page";
            $sth = $this->db->prepare($query);
            $sth->execute();
            $data = $sth->fetchAll(PDO::FETCH_ASSOC);
            $sth->closeCursor();
